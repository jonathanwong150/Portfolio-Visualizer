...
- **Partial ETF constituent data must never be normalized into certainty.** `engine.py` uses reported weights verbatim and emits uncovered value as `UNRESOLVED:<fund>`. Negative/non-finite weights or a total materially over 100% make the entire fund unresolved. Keep unresolved rows out of named-company rankings and factor scores while retaining them in portfolio and breakdown totals. (2026-09-09)
