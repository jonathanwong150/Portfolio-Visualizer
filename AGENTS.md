...
The subtle part: the same company arrives from several ETFs *and* possibly a direct holding, and all of it must sum into one row. Partial constituent lists keep their reported weights verbatim; uncovered value is a separate `UNRESOLVED:<fund>` row and never inflates named companies.
...
- **Partial ETF constituent data must never be normalized into certainty.** `engine.py` uses reported weights verbatim and emits uncovered value as `UNRESOLVED:<fund>`. Any invalid or overweight list makes the entire fund unresolved, and the internal `(is_unresolved, ticker)` identity prevents that display namespace from colliding with a real ticker. Keep unresolved rows out of named-company rankings and factor scores while retaining them in portfolio and breakdown totals. (2026-09-09)
- **Market-refresh tests passed only when a developer happened to have an Alpha Vantage key.** Stubbed refresh tests must pin `Settings.alphavantage_configured` explicitly, including the negative case, so a clean checkout verifies without `.env`. (2026-09-09)
